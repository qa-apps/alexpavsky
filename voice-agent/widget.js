 ...
 *   window.VOICE_AGENT_CHAT_OPEN_POS = {                    // offset while #chat-window is open
 *     right: "calc(min(560px, 50vw) + 48px)", bottom: 116
 *   }
...
(function () {
  ...
  var CHAT_OPEN_POS = window.VOICE_AGENT_CHAT_OPEN_POS || {
    right: 'calc(min(560px, 50vw) + 48px)', bottom: 116
  };
  ...
  var CLOSED_RIGHT = cssLength(POS.right, 20);
  var CLOSED_BOTTOM = cssLength(POS.bottom, 148);
  var OPEN_RIGHT = cssLength(CHAT_OPEN_POS.right, 608);
  var OPEN_BOTTOM = cssLength(CHAT_OPEN_POS.bottom, 116);
  ...
  function updateContextPosition() {
    ...
    var forum = document.getElementById('forum-open-btn');
    var chatOpen = !!chat && chat.classList.contains('active');
    host.classList.toggle('chat-open', chatOpen);

    if (!chatOpen || !forum) {
      host.style.removeProperty('--va-open-bottom');
      return;
    }

    var forumRect = forum.getBoundingClientRect();
    var forumVisible = forumRect.top >= 0 && forumRect.top < window.innerHeight;
    if (forumVisible) {
      host.style.setProperty('--va-open-bottom', (window.innerHeight - forumRect.top + 14) + 'px');
    } else {
      host.style.removeProperty('--va-open-bottom');
    }
  }

  function scheduleContextPosition() {
    if (positionRaf) return;
    positionRaf = window.requestAnimationFrame(updateContextPosition);
  }

  function syncChatPosition() {
    scheduleContextPosition();
  }

  function watchChatPosition() {
    var chat = document.getElementById('chat-window');
    syncChatPosition();
    if (!chat || !window.MutationObserver) return;
    new MutationObserver(syncChatPosition).observe(chat, {
      attributes: true,
      attributeFilter: ['class']
    });
    window.addEventListener('scroll', scheduleContextPosition, { passive: true });
    window.addEventListener('resize', scheduleContextPosition);
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', watchChatPosition, { once: true });
  } else {
    watchChatPosition();
  }
  ...
  function CSS() {
    return [
      ':host{ all:initial; --va-right:' + CLOSED_RIGHT + '; --va-left:auto; --va-bottom:' + CLOSED_BOTTOM + '; }',
      ':host(.chat-open){ --va-right:' + OPEN_RIGHT + '; --va-bottom:var(--va-open-bottom, ' + OPEN_BOTTOM + '); }',
      ...
      /* ---------- launcher ---------- */
      '.launcher{ position:fixed; right:var(--va-right); left:var(--va-left); bottom:var(--va-bottom); z-index:2147483000;',
      ...
      '  transition:right .38s cubic-bezier(.2,.8,.2,1), left .38s cubic-bezier(.2,.8,.2,1), bottom .38s cubic-bezier(.2,.8,.2,1), transform .25s cubic-bezier(.2,.8,.2,1), box-shadow .3s, opacity .3s;',
      ...
      /* ---------- voice stage ---------- */
      '.stage{ position:fixed; right:clamp(14px, var(--va-right), calc(100vw - 354px)); left:var(--va-left); bottom:var(--va-bottom); z-index:2147483000;',
      ...
      '  transition:right .38s cubic-bezier(.2,.8,.2,1), left .38s cubic-bezier(.2,.8,.2,1), bottom .38s cubic-bezier(.2,.8,.2,1), opacity .28s ease, transform .32s cubic-bezier(.2,.85,.25,1); }',
      ...
      '  :host(.chat-open){ --va-right:auto; --va-left:14px; --va-bottom:var(--va-open-bottom, max(24px, calc(env(safe-area-inset-bottom) + 20px))); }',
      '  .launcher{ max-width:calc(100vw - 28px); }',
      '  :host(.chat-open) .launcher{ padding:8px; }',
      '  :host(.chat-open) .l-label{ display:none; }',
      '  .stage{ right:14px; left:14px; width:auto; transform-origin:bottom center; }',
      '}',
